...
#TODO: a ser ejecutado cada 30 min
every(1.minutes, 'finish_and_pay_appointments', :at => ['**:30', '**:00']) {
  ...
  # TODO: cambiar a 12 horas
  Appointment.where("appointments.appointment_status_id = ? AND appointments.charged = ? AND appointments.paid = ? AND appointments.end < ?", completed_appointment.id, false, false, Time.now + 1.minute).each do |appointment|
